...
Unluckily, \Metis does not make explicit the way it uses the subgoals
to prove the goal by the time this document is written. Thus we show the
explicit description of this stripping process in the correctness of
the \strip inference rule in Theorem~\ref{thm:strip}. We first need
to prove Lemma~\ref{lem:inv-uh-lem} and Lemma~\ref{lem:lem-inv-strip}.
