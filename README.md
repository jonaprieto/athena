...
First, we write the problem ![problem](//tex.s2cms.ru/png/%24%24%5C%7B%20p%20%2C%20q%5C%7D%20%5Cvdash%20p%20%5Cwedge%20q%24%24) in TPTP to generate the solution using the Metis prover.
...
Get a proof using the Metis prover by running the following commands:
...
Now, we generate the Agda code using Athena:

```
$ athena problem.tstp
```

The Agda file is named with the same name of the TSTP solution:
...
The last step is type-checking of the proof. At this stage, we will assume
...
an interface file of the type-checking. At the end, we should have four files: the TPTP problem, the TSTP solution, the Agda proof-term, and the Agda interface file.
