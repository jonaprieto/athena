...
\begin{frame}[fragile]{Proposition Type}
...
\item Constructors (inference rules)

\begin{MyAgda}
assume, ⊤-intro, ⊥-elim, ¬-intro,
¬-elim, ∧-intro, ∧-proj₁, ∧-proj₂, ∨-intro₁,
∨-intro₂, ∨-elim, ⇒-intro, ⇒-elim, ⇔-intro,
⇔-elim₁, ⇔-elim₂.
\end{MyAgda}
For example, the introduction rule for conjunction ∧-intro is
the constructor:
\begin{MyAgda}
  ∧-intro
    : ∀ {Γ} {φ ψ}
...
\end{MyAgda}
  ...
  \item In \cite{AgdaProp} we can find more than 99 theorems to reasoning
  in classical propositional logic

\begin{MyAgda}
∧-assoc, ∧-comm, ∧-dist, ¬∧-to-¬∨¬, ∨-assoc, ∨-comm, ∨-dist, ∨-equiv
¬¬∨¬¬-to-∨, cnf, nnf, dnf, ...
\end{MyAgda}
...
\Procedure{metis}{}\newline
  ...
  \textbf{output:} maybe a derivation when $a_1,\cdots,a_n \vdash \textrm{goal}$, otherwise nothing. \newline
    ...
    \ \ \  \If{a contradiction can be derived the assumptions}
    \ \ \ \ \ \State{keep the refutation and continue with the others subgoals}
   \ \ \ \ \ \Else
    \ \ \ \ \ \State{exit without a proof. The conjecture can not be derived from the premises}
    \EndIf
    \EndFor
...
% \begin{frame}[fragile, label=agda-file]{\Agda Code Example}
%   {Generated by \Athena Tool}
...
% \begin{frame}[fragile, label=verified-example]{Problem $p ∧ q ⊢ q ∧ p$}{Proof generated with \Athena}
% \vfill
% \begin{minted}{cagda}
