...
\begin{frame}{Why Metis?}
  \begin{itemize}
    \item \href{https://github.com/gilith/metis}{Open source} implemented in \texttt{Standard ML}
    \item Each refutation step is one of the three rules
...
\begin{frame}[fragile]{Refutation Tree for the Subgoal No. 1: $(p ∨ q) ⇒ p$}
...
\begin{frame}[fragile]{Refutation of Subgoal 2}
\begin{tptp}
fof(s2, ((p ∨ q) ∧ p) ⇒ q, inf(strip,goal)).
fof(neg2, ¬ (((p ∨ q) ∧ p) ⇒ q), inf(negate,s2)).
fof(n10, ¬ q ∧ p ∧ (p ∨ q), inf(canonicalize, neg2)).
