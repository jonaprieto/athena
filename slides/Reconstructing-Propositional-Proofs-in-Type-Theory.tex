...
\begin{frame}[fragile]{\Metis Refutation Strategy}
\begin{algorithm}[H]
    % \caption{Perturbación de una solución}
    \begin{algorithmic}[Metis]
        \Procedure{metis}{}\newline
        \textbf{input:} a \textit{goal} and a set of \emph{premises} $a_i$\newline
        \textbf{output:} maybe a derivation when $\{a_i\} \vdash goal$, otherwise nothing.
        \State{Strip the goal into a list of \emph{subgoals}.}
        \For{each subgoal $s_i$}
        \State{Try to find by a refutation of $\neg s_i$}
        \EndFor

        % \If{si el número de la iteración numIter en Alg.~1 es un múltiplo $l$}
        %     \State{$p_k \leftarrow$ seleccione aleatoriamente un parámetro de $c_{+}$}
        % \Else
        %     \State{$p_k \leftarrow$ seleccione aleatoriamente un parámetro de $c_{-}$}
        % \EndIf
        % \State{modifique el parámetro $p_k$ en una proporción $\lambda$}
        % \State{retorne el nuevo conjunto de parámetros}
        \EndProcedure
    \end{algorithmic}
    \label{alg_2}
\end{algorithm}
