...
\item Definition
\begin{equation*}
\text{\color{plum!40!black}conjunct}(\overbrace{ϕ₁ ∧ ⋯ ∧ \phi_n}^{\phi}, \psi)
= \begin{cases}
\phi_{i} \ \ \ \ \text{if }\psi\text{ is equal to some }\phi_i\\
\phi    \ \ \ \ \text{otherwise}
\end{cases}
\end{equation*}
...
%  , node distance=2.8cm
  ...
  \tikzset{shift={(current page.center)}, xshift=4.5cm,yshift=1cm}
    

  \node[state]  (phi)                     {$φ$};
  \node[state]  (phi1) [below left =1.3cm and 0.1cm of phi]  {$φ₁$};
  \node[state]  (phi2) [below right=1.3cm and 0.1cm of phi]  {$φ₂$};
  \node[state]  (phi3)  [below left =1.3cm and 0.1cm of phi2]  {$φ₃$};
  \node[state]  (phi4)  [below right =1.3cm and 0.1cm of phi2]  {$φ₄$};
...
\item
\texttt{conjunct}$(φ , φ₃ ∧ φ₁) ≡ ϕ$
\vskip 3mm
\item
\texttt{conjunct}$(φ , φ₃)      ≡ ϕ₃$
\vskip 3mm
\item
\texttt{conjunct}$(φ , φ₂)      ≡ ϕ₂$
\vskip 3mm
...
... | _          | []           = φ
... | conj _ _ | pick  ∷ path = φ
... | conj φ₁ _ | proj₁ ∷ path = conjunct φ₁ ψ
... | conj _ φ₂ | proj₂ ∷ path = conjunct φ₂ ψ
... | other .φ   | _            = φ
...
atp-conjunct {Γ} {φ} Γ⊢φ ψ
  with conj-view φ | conjunct-path φ ψ []
...  | _          | []           = Γ⊢φ
...  | conj φ₁ φ₂ | pick  ∷ path = Γ⊢φ
...  | conj φ₁ φ₂ | proj₁ ∷ path = atp-conjunct ψ (∧-proj₁ Γ⊢φ)
...  | conj φ₁ φ₂ | proj₂ ∷ path = atp-conjunct ψ (∧-proj₂ Γ⊢φ)
...  | other .φ   | (_ ∷ _)      = Γ⊢φ
\end{minted}
\vfill
