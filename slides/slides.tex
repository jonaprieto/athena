...
\item Definition
\begin{equation*}
\text{\color{plum!40!black}conjunct}(ϕ₁ ∧ ⋯ ∧ \phi_n, \omega)
= \begin{cases}
\phi_{i} \ \ \ \ \text{if }\omega\text{ is equal to some }\phi_i\\
\phi    \ \ \ \ \text{otherwise}
\end{cases}
\end{equation*}
...
\node[vertex, fill=black, below left=0.2cm and 0.1cm of d1] (d2) {};
\node[vertex, fill=black, below right=0.2cm and 0.1cm of d1] (d3) {};

\node[vertex, fill=black, below left=0.2cm and 0.1cm of d3] (q) {};
\node[vertex, fill=black, below right=0.2cm and 0.1cm of d3] (w) {};

\node[vertex, fill=black, below right=0.2cm and 0.1cm of q] (s) {};
...
\begin{frame}[fragile, label=atp-conjunct]
...
...   | other _   = []
...   | conj φ₁ φ₂ with conjunct-path φ₁ ω []
...
atp-conjunct {Γ} {φ} ω Γ⊢φ
  with conj-view φ | conjunct-path φ ω []
...  | _          | []           = Γ⊢φ
...
\end{tptp}
  ...
  \vspace*{5mm}
\begin{tikzpicture}
\node[rectangle, text=white,fill=black, align=left, font=\small]
{
\texttt{\$ metis --show proof problem.tptp > problem.tstp}\hspace{1cm}\\
\texttt{\$ athena problem.tstp}\\
\texttt{\$ agda problem.agda}
  };
\end{tikzpicture}
