...
\begin{frame}[fragile, label=tptp-examples]{TPTP Examples}{From Prop-Pack repository
  \footnote{\url{http://github.com/jonaprieto/prop-pack}}}
...
\hyperlink{tstp-example}{TSTP} derivations by Metis exhibit these inferences
...
where \textbf{\color{plum} source} typically is an inference record:
\begin{center}
{\footnotesize
\begin{tptp}
...
\begin{frame}[fragile, label=tstp-dag]{DAG for the previous TSTP derivation found by \texttt{Metis} Prover}
...
\begin{prooftree}
\AxiomC{$p$}
\RightLabel{\texttt{assume}}
\UnaryInfC{$p$}
\RightLabel{\texttt{strip}}
\UnaryInfC{$p$}
\RightLabel{\texttt{negate}}
\UnaryInfC{$¬ p$}
\AxiomC{$p$}
\RightLabel{\texttt{canonicalize}}
\UnaryInfC{$p$}
\RightLabel{\texttt{simplify}}
\BinaryInfC{$⊥$}
\RightLabel{\texttt{canonicalize}}
\UnaryInfC{$⊥$}
...
Is a Haskell program
that translates proofs given by Metis Prover
in \texttt{TSTP} format to Agda code.\\
...
\hline
\texttt{\hyperlink{agda-prop}{Agda-Prop}}& axioms and theorems of Classical Propositional Logic\\
\texttt{\hyperlink{agda-metis}{Agda-Metis}} & versions of the inference rules used by Metis\\
\hline
\end{tabular}}
}
\end{center}
...
\texttt{strip} & splits a formula into subgoals &\texttt{strip}\\[1ex]
\hline
\end{tabular}}
}
\end{center}
...
\begin{frame}[fragile, allowframebreaks]{TSTP derivation}
\begin{tptp}
fof(a, axiom, p & q).
fof(goal, conjecture, q & p).
...
\begin{frame}[fragile, label=failure-example]{Bug in the Printing of the Proof}
  {Metis' Issue: \url{https://github.com/gilith/metis/issues/2}}
% {}
% \begin{tptp}
% $ cat problem.tptp
% fof(goal, conjecture,
