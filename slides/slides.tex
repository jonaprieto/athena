...
\newcommand{\gobackreconstruction}{
  \begin{tikzpicture}[overlay, remember picture, scale=0.5]
    \tikzset{shift={(current page.center)}, xshift=11.6cm,yshift=-8.3cm}
...
\begin{itemize}
  \item Is a \hyperlink{tstp-dag}{Directed Acyclic Graph} where
  \begin{itemize}
    \item[\texttt{leaf}] is a formulae from the \hyperlink{tptp-syntax}{TPTP} input
...
With Classical Propositional Logic, \hyperlink{tstp-example}{TSTP} derivations exhibit
these inferences:
\begin{itemize}
\item \hyperlink{atp-canonicalize}
  {\texttt{Canonicalize} transforms formulas to CNF or NNF}
\item \hyperlink{atp-clausify}
  {\texttt{Clausify} performs clausification}
\item \hyperlink{atp-conjunct}
  {\texttt{Conjunct} extracts a formula from a conjunction}
\item \hyperlink{atp-negate}
  {\texttt{Negate} applies negation to the formula}
\item \hyperlink{atp-resolve}
  {\texttt{Resolve} applies theorems of resolution}
\item \hyperlink{atp-simplify}
  {\texttt{Simplify} applies over a list of formula to simplify them}
% \item[\texttt{Skolemize}] ...
% \item[\texttt{Specialize}] ...
\item \hyperlink{atp-strip}{\texttt{Strip} splits a formula into subgoals}
\end{itemize}
\gobackreconstruction
\end{frame}
...
\begin{itemize}
  \item Add shallow embedding in order to work with Apia
  \item Support First-Order Logic
  \item Support another prover like EProver
  \item Support a Intuitionistic Prover
\end{itemize}
\end{frame}
