...
the $\fnnfp$ function. Its definition is based on the \Metis source code
to normalize formulas. Nevertheless, we follow the same idea to
...
the function. Therefore, we define the \abbre{Polarity} type with
two constructors: $⊕$ to denote positive polarity, and $⊖$ to denote
negative polarity of a formula. The formula polarity of a formula was not
relevant since as we see later, we found using the $⊕$ polarity to call
\fpolarity function is enough for propositions.
