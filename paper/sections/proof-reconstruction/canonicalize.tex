...
\canonicalize rule in these cases uses the conjunctive normal form
with simplifications. Otherwise, when an axiom, definition or
hypothesis is needed to prove some goal, this rule gets the negative
normal form of the formula. The \canonicalize rule jointly with the
\clausify rule perform the so-called \emph{clausification} process
mainly described in~\cite{Sutcliffe1996}.
...
simplifications by applying to it the $\fnnf_{0}$ function defined
in~\eqref{eq:nnf}. This definition is mainly based on the \Metis
source code to normalize formulas.

To define such a function in type theory we used bounded recursion
described in Section~\ref{ssec:structural-recursion}.

\begin{equation}
\label{eq:nnf}
\begin{aligned}
&\hspace{.495mm}\fnnfp : \Prop \to \Nat \to \Prop\\
&\begin{array}{llll}
\fnnfp &(φ₁ ∧ φ₂)     &(n+1) &= \fcanon_{∧}~(\fassoc_{∧}~(\fnnfp~φ₁~n ∧ \fnnfp~φ₂~n))\\
\fnnfp &(φ₁ ∨ φ₂)     &(n+1) &= \fcanon_{∨}~(\fassoc_{∨}~(\fnnfp~φ₁~n ∨ \fnnfp~φ₂~n))\\
\fnnfp &(φ₁ ⇒ φ₂)     &(n+1) &= \fcanon_{∨}~(\fassoc_{∨}~(\fnnfp~((¬ φ₁) ∨ φ₂)~n))\\
\fnnfp &(¬ (φ₁ ∧ φ₂)) &(n+1) &= \fcanon_{∨}~(\fassoc_{∨}~(\fnnfp~((¬ φ₁) ∨ (¬ φ₂))~n))\\
\fnnfp &(¬ (φ₁ ∨ φ₂)) &(n+1) &= \fcanon_{∧}~(\fassoc_{∧}~(\fnnfp~((¬ φ₁) ∧ (¬ φ₂))~n))\\
\fnnfp &(¬ (φ₁ ⇒ φ₂)) &(n+1) &= \fnnfp~φ₁~n\\
\fnnfp &(¬ (¬ φ))     &(n+1) &= \fcanon_{∧}~(\fassoc_{∧}~(\fnnfp~((¬ φ₂) ∧ φ₁)~n))\\
\fnnfp &(¬ ⊤)         &(n+1) &= ⊥\\
\fnnfp &(¬ ⊥)         &(n+1) &= ⊤\\
\fnnfp &φ             &0     &= φ
\end{array}
...
\begin{mainlemma}
  ...
  \begin{align*}
   \begin{split}
     &\fnnf : \NProp \to \NProp\\
     &\fnnf~φ = \fnnfp~φ~(\fnnf_{cm}~φ).
   \end{split}
  \end{align*}
  and $\fnnf_{cm}~φ$ is defined in Appendix~\ref{app:polarity-for-propositions}.
\end{mainlemma}
