...
We emulate each \Metis inference rule by proving a
function with the same name, and a theorem that
proves the soundness of the function.
The pattern is the following.

\begin{itemize}
\item \texttt{rule} is the \Metis inference rule.
\item $rule$ is the emulated version function for \texttt{rule} inference.
\begin{align*}
\begin{split}
&rule : \Prop → \Prop → \Prop\\
&rule(φ, ψ) =
...
In the following \Metis \TSTP derivation, we saw a clear example, where
...
\label{thm:thm-clausify}
  $Γ ⊢ φ$ and $ψ \in\Prop$ then $Γ ⊢ clausify(φ, ψ)$
\end{theorem}
