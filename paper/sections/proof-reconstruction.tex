...
Then, we define the $rm_{∨}$ function to remove in a disjunction φ, those propositions ψ repeated in the formula.
...
Finally, we remove redundancies in a proposition in its conjunctive normal form, by composing the functions described above in the Eq.~\ref{eq:rm-cnf} for the function $cleanCNF$.

\begin{equation*}
  \label{eq:rm-cnf}
  cleanCNF = rm_{∧} ∘ rm_{∧∨} ∘ rconj ∘ cnf.
\end{equation*}
