...
As a result of using \Athena, we get an \Agda file of the proof.
To generate this file, \Athena uses names of functions and theorems from
our \Agda libraries: \texttt{agda-prop}, and \texttt{agda-metis}.
...
investigated by the user looking at the error in the \TSTP derivation by \Metis,
in the translation by \Athena,in the \Agda formalisations mentioned above or
even in the type-checker, \Agda..
...
usual, i.e., \zero and \suc are the data constructors. We use names
...
In this section, we emulate each \Metis inference rule with
function and its respective theorem.
The following pattern is used to present the formalization.
The function name written in bold face
...
$\rm{rule}$ function.
\begin{equation*}\text{If }Γ ⊢ φ\text{ then }Γ ⊢ rule\ φ.\end{equation*}
...
\label{def:strip}
  \begin{align}
  \label{eq:strip}
    &\fstrip &&: \Prop → \Nat → \Prop\\
    &\fstrip\ (φ₁ ∧ φ₂)\ n     &&= \fuh\ (\fstrip\ φ₁) ∧ \fuh\ (φ₁ ⇒ \fstrip\ φ₂)\\
    &\fstrip\ (φ₁ ∨ φ₂)\ n     &&= \fuh\ (¬ φ₁ ⇒ \fstrip\ φ₂)\\
    &\fstrip\ (φ₁ ⇒ φ₂)\ n     &&= \fuh\ (φ₁ ⇒ \fstrip\ φ₂)\\
    &\fstrip\ (φ₁ ⇔ φ₂)\ n     &&= \fuh\ φ₁ ⇒ \fstrip\ φ₂) ∧ \fuh\ φ₂ ⇒ \fstrip φ₁)\\
    &\fstrip\ (¬ (φ₁ ∧ φ₂))\ n &&= \fuh\ φ₁ ⇒ \fstrip(¬ φ₂))\\
    &\fstrip\ (¬ (φ₁ ∨ φ₂))\ n &&= \fuh\ \fstrip(¬ φ₁)) ∧\ \fuh ¬ φ₁ ⇒ \fstrip(¬ φ₂))\\
    &\fstrip\ (¬ (φ₁ ⇒ φ₂))\ n &&= \fuh\ \fstrip(φ₁)) ∧\ \fuh φ₁ ⇒ \fstrip(¬ φ₂))\\
    &\fstrip\ (¬ (φ₁ ⇔ φ₂))\ n &&= \fuh\ φ₁ ⇒ \fstrip(¬ φ₂)) ∧\ \fuh ¬ φ₂ ⇒ \fstrip(φ₁))\\
    &\fstrip\ (¬ ¬ φ₁)\ n      &&= \fuh\ \fstrip(φ₁))\\
    &\fstrip\ (¬ ⊥)\ n         &&= ⊤\\
    &\fstrip\ (¬ ⊤.)\ n        &&= ⊥\\
    &\fstrip\ φ\ n             &&= φ
  \end{align}
