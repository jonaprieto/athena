...
theorems. In the latter type, $Γ$ has $\abbre{List}~\Prop$ type
...
by converting propositions to their conjunctive normal forms and showing
equivalence by reordering the conjunctions and the inner disjunctions.
In this section, we provide some theorems to perform such reordering tasks,
the goals are the following.

\begin{itemize}
  \item Reorder the disjunction $φ = φ₁ ∨ φ₂ ∨ \cdots φₙ$ to match with
...
First, we define the $rdisj$ function in Eq.~(\ref{eq:rdisj-def}) to convert
...
\begin{definition}[simpl]
\label{def:simpl}
\begin{align}
    \begin{split}
      &\textit{simpl} : \Prop → \Prop → \Prop\\
      &\textit{simpl}(φ₁, φ₂) =
      \begin{cases}
        ⊥,  &\text{ if }φᵢ ≡ ⊥,\\
        γ   &\text{ if }φᵢ ≡ φⱼ ⇒ γ\\
        ...
        γ   &\text{ if }φᵢ ≡ φⱼ ⇔ γ \\
        γ   &\text{ if }φᵢ ≡ ¬ φⱼ ∨ γ\\
        ⊥   &\text{ if }conjunct(φᵢ, ¬ φⱼ)\\
        ⊥,  &\text{ if }canonicalize(φᵢ, ¬ φⱼ) ≡ ¬ φⱼ\\
        φ₁, &\text{ otherwise.}\\
      \end{cases}
    \end{split}
\end{align}
For $i = 1, 2$, $i\neq j$. We assume here some commutative properties for brevity.
\end{definition}
