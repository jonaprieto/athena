...
Let us consider the following example from a inner function
...
Actually, it maintains with the same length syntactically
(i.e., (φ₁ ∧ φ₂) ⇒ φ₃ compared with (φ₁ ⇒ (φ₂ ⇒ φ₃))).
Therefore, a termination checking will fail with \unshunt function.
...
the formula that \unshunt function transforms.
...
$\unshunt_{cm}: \PROP → \NN$
for the second argument of the $\unshunt$ function in
...
Then, a curated version of the \unshunt functions is presented in Eq.
...
subsection~\ref{ssec:well-structured recursion}, since
...
\begin{theorem} If $s_i$ are the subgoals for the goal φ, and $\{ Γ ⊢ s_i \}_{i}$, then $Γ ⊢ φ$.
\end{theorem}
