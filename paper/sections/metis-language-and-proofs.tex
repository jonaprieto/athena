...
the customized \TSTP syntax. Note that we remove for example
empty fields in the inference information, the \verb!role! field if it is
\verb!plain! and shorten names as long as possible.
...
% \TSTP derivations replied by \Metis use the following rules:
...
in Fig.~\ref{fig:metis-proof-tstp}).
