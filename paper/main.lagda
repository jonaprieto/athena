...
Despite the six simple rules in the \verb!Metis!'s logic kernel, in the
...
derivations. The literal must occur in the first theorem, and its negation 
must occur in the second theorem (see the \verb!Metis!'s logic core in
...
\[%\scalebox{0.9}{
\begin{bprooftree}
\AxiomC{$\delta_{\phi} := \Gamma \vdash p\vee \phi$}
\AxiomC{$\delta_{\psi} := \Gamma \vdash \neg p \vee \psi$}
...
\end{code}
\caption{Excerpt of an example of \verb!resolve! rule in a \verb!TSTP! 
derivation.}
...
\textit{Negate.} Since \verb!Metis!'s proofs are refutations, for each 
...
the logic constants $\{\top, \bot\}$ and logical connectives $\{\wedge, \vee
, \Rightarrow, \Leftrightarrow, \neg\}$. In \verb!Agda!, we define the 
formula
...
Then, our propositional formulas are represented using the \verb!Prop! data 
type. %an extension of the syntax defintion used in \cite{Altenkirch2015}.
